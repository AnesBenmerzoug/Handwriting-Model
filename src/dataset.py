...
class IAMDataset(Dataset):
    ...
    def __init__(self, parameters):
        ...
        self.data_filename = os.path.join(self.params.DatasetDir, 'strokes_data.pickled')
        ...
        self.alphabet = ''.join([' '] + [c for c in string.ascii_lowercase + string.ascii_uppercase])
        self.length = 0
        self.limit = 300
    ...
    def prepocess_data(self):
        ...
        def create_data_path_list():
            ...
            ascii_dir = os.path.join(self.params.DatasetDir, 'ascii')
                    ...
                    if os.path.isdir(strokes_dir):
                        for stroke_file in os.listdir(strokes_dir):
                            if f[:-4] in stroke_file:
                                stroke_paths.append(os.path.join(strokes_dir, stroke_file))
    ...
    def load_data(self):
        ...
        for sentence, stroke in zip(raw_ascii, raw_strokes):
            ...
            onehot = np.zeros(shape=(len(sentence), len(self.alphabet)+1), dtype=np.uint8)
        ...
        self.strokes[:, 1:, :2] = self.strokes[:, 1:, :2] / np.std(self.strokes[:, 1:, :2], axis=(0, 1))
